...
func connnectDB(cfg *conf.Data_Database) *gorm.DB {
	if cfg == nil {
...
func main() {
	...
	g.UseDB(connnectDB(bc.Data.Database))

	// 从连接的数据库为所有表生成Model结构体和CRUD代码
	// 也可以手动指定需要生成代码的数据表
	g.ApplyBasic(g.GenerateAllTable()...)
	//自定义的查询逻辑
	//g.ApplyInterface(func(query selfQuery) {}, g.GenerateModel("todos"))
	// 执行并生成代码
	g.Execute()
}
